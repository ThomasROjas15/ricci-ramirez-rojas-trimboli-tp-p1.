			...
			if (prisionero != null) { //si el prisionero no es un objeto inexistente o null sus funciones se ejecutaran sin afectar el juego
				...
				if (entorno.estaPresionada(entorno.TECLA_DERECHA) || entorno.estaPresionada(entorno.TECLA_IZQUIERDA)
					...
					if (bolaDePoder == null && entorno.estaPresionada(entorno.TECLA_ESPACIO)) {
						bolaDePoder = new BolaDePoder(prisionero.x, prisionero.y, prisionero.dir);
					}
...
//			genera 2 soldados aleatoriamente si hay menos de dos soldados en pantalla
			while (soldados.size() < 2) { 
		           int x = rand.nextInt(2) == 0 ? 20 : 780; //operador ternario, Posición x aleatoria a la izquierda o derecha de la pantalla 
		           int y = rand.nextInt(entorno.alto() - 100); // Posición y aleatoria dentro de los límites de la pantalla 
		           soldados.add(new Enemigo(x, y));
